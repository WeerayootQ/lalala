...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        [self setup];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideIntroSignInWithNotification:) name:@"HIDE_INTRO_SIGNIN" object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideIntroSignUpWithNotification:) name:@"HIDE_INTRO_SIGNUP" object:nil];
    }
    return self;
}
...
{
    NSIndexPath *nextIndex = (NSIndexPath *)[self.collectionView indexPathsForVisibleItems][0];
    nextIndex = [NSIndexPath indexPathForRow:nextIndex.row + 1 inSection:nextIndex.section];
    NSInteger nPages = [self.dataSource numberOfPages];
    if (nextIndex.row >= nPages)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SHOW_ACTIONSHEET" object:nil];
    }
    else
    {
        [self.collectionView scrollToItemAtIndexPath:nextIndex
                                    atScrollPosition:UICollectionViewScrollDirectionVertical
                                            animated:YES];
        self.pageControl.currentPage = ((NSIndexPath*)[self.collectionView indexPathsForVisibleItems][0]).row + 1;
    }
}
...
- (void)hideIntroSignInWithNotification:(NSNotificationCenter *)notification
{
    [UIView animateWithDuration:0 animations:^{
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            // TODO
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"SHOW_SIGNIN" object:nil];
        });
	}];
}

- (void)hideIntroSignUpWithNotification:(NSNotificationCenter *)notification
{
    [UIView animateWithDuration:0 animations:^{
        self.alpha = 1;
    } completion:^(BOOL finished){
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            [self removeFromSuperview];
            // TODO
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"SHOW_SIGNUP" object:nil];
        });
	}];
}
